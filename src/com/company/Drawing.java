...
public class Drawing extends Canvas {
    public static void start (String[] command) {
        for (int i = 1; i < command.length; i++) {//if 't'(time between iterations) or 'i'(number of iterations) is specified, the corresponding value is changed to what is specified
    ...
    public void paint(Graphics g) {
        double[] coordinatesAndColor = {0.0, 0.0, Main.generatorValues[0][7]};//beginning coordinates are 0 and beginning color is that of the first function
        ...
        for (int i = 0; true; i++) {
            ...
            g.setColor(color);
            System.out.println(coordinatesAndColor[0]+","+coordinatesAndColor[1]);//coordinates are written as output

            if (Main.totalIterations>0&&Main.totalIterations<i) {//if the iterations have expired
                break;
            }
